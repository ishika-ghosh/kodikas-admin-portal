...
function Scanner() {
    ...
    useEffect(() => {
        ...
        async function onScanSuccess(qrCodeMessage) {
            ...
            const eventDataResponse = await fetch(`/api/events/teamDetail?teamid=${qrCodeMessage}`);
                    ...
                    <>
                        <div className="h-screen  -ml-32 flex justify-center flex-col gap-6">
                            <div className="flex items-center mb-4">
                                <input
                                    type="checkbox"
                                    disabled={eventData?.attendance || !checkTime("Attendance")}
                                ...
                                <label className="ml-8 font-extrabold text-3xl text-purple-600">
                                    {eventData?.attendance ? <s className="text-gray-600">Entry</s> : !checkTime("Attendance") ? <p className="text-gray-600">Entry</p> : "Entry"}
                                </label>
                            </div>
                            <div className="flex items-center mb-4">
                                <input
                                    type="checkbox"
                                    disabled={eventData?.lunch || !checkTime("Lunch") || !eventData?.first}
                                    defaultChecked={eventData?.lunch}
                                    className="w-8 h-8 accent-purple-600 rounded"
                                    onChange={() => setLunchStatus((prev) => !prev)}
                                />
                                <label className="ml-8 font-extrabold text-3xl text-purple-600">
                                    {eventData?.lunch ? <s className="text-gray-600">Lunch</s> : !checkTime("Lunch") ? <p className="text-gray-600">Lunch</p> : "Lunch"}
                                </label>
                            </div>
                            <button
                                type="button"
                            ...
                            <>
                                {teamData ? (
                                    <div className="h-screen -ml-32 flex items-center justify-center flex-col gap-5">
                                        <Updates payment={paymentStatus} attendance={entryStatus} first={eventData?.first} lunch={lunchStatus} second={eventData?.second} />
